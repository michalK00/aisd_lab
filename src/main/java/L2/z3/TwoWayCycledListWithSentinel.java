...
public class TwoWayCycledListWithSentinel<E> extends AbstractList<E> {
    ...
    public TwoWayCycledListWithSentinel mergeAnotherListIntoCurrentOneMethod1(TwoWayCycledListWithSentinel list, int index){
        if(this.size()<index){
            System.out.println("Error - indeks przekracza zakres");
            return this;
        } else if(this.size()<index){
            mergeAnotherListIntoCurrentOneMethod3(list);
        }

        Element firstElementOfInput = list.sentinel.getNext();
        Element lastElementOfInput = list.sentinel.getPrev();
        Element elementOfCurrentListAfterInput = this.getElement(index);
        Element elementOfCurrentListBeforeInput = this.getElement(index-1);

        elementOfCurrentListBeforeInput.setNext(firstElementOfInput);
        elementOfCurrentListAfterInput.setPrev(lastElementOfInput);
        firstElementOfInput.setPrev(elementOfCurrentListBeforeInput);
        lastElementOfInput.setNext(elementOfCurrentListAfterInput);

        list.clear();
        return this;

    }
    public TwoWayCycledListWithSentinel mergeAnotherListIntoCurrentOneMethod2(TwoWayCycledListWithSentinel list, E object){
        Element element = sentinel.getNext();
        int index = 0;
        while(element!=sentinel && !object.equals(element.getValue())){
            index++;
            element=element.getNext();
        }
        if(element==sentinel){
            System.out.println("Error - nie ma takiego obiektu w liście");
            return this;
        }
        return mergeAnotherListIntoCurrentOneMethod1(list,index);

    }
    public TwoWayCycledListWithSentinel mergeAnotherListIntoCurrentOneMethod3(TwoWayCycledListWithSentinel list){

        Element firstElementOfInput = list.sentinel.getNext();
        Element lastElementOfInput = list.sentinel.getPrev();
        Element lastElementOfCurrentList = this.sentinel.getPrev();

        lastElementOfCurrentList.setNext(firstElementOfInput);
        lastElementOfInput.setNext(this.sentinel);
        firstElementOfInput.setPrev(lastElementOfCurrentList);
        this.sentinel.setPrev(lastElementOfInput);
